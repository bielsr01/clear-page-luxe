...
export function LoyaltyPanel({ restaurantId, isAdmin = false }: { restaurantId: string; isAdmin?: boolean }) {
  ...
  const { data: restaurant } = useQuery({
    queryKey: ["restaurant-slug", restaurantId],
    queryFn: async () => {
      const { data } = await sb.from("restaurants").select("slug").eq("id", restaurantId).single();
      return data;
    },
  });

  const loyaltyLink = useMemo(() => {
    if (!restaurant?.slug) return "";
    return `${window.location.origin}/fidelidade/${restaurant.slug}`;
  }, [restaurant?.slug]);
